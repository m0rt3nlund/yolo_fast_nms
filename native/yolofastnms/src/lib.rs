...
fn run_with_binary<'a>(
    ...
    let filtered_bboxes = bboxes
        .into_iter()
        .filter(|b| b.prob >= prob_threshold)
        .collect();
    ...
    let result: Vec<Vec<f32>> = final_bboxes
        .into_iter()
        .map(|bbox| {
            let mut vec = vec![
                bbox.cx as f32,
...
fn binary_to_matrix(binary: &Binary, rows: usize, columns: usize) -> Vec<Vec<f32>> {
    ...
    assert!(
        binary.len() == total_size,
        "Binary size ({}) is less than the total size ({})",
        binary.len(),
        total_size
    );
    assert!(
        binary.len() >= row_size,
        "Binary size ({}) is less than the row size ({})",
        binary.len(),
        row_size
    );
    assert!(
        binary.len() % row_size == 0,
        "Binary size ({}) is not a multiple of the row size ({})",
        binary.len(),
        row_size
    );
...
fn bbox_from_row(row: &Vec<f32>, classes: usize) -> BBox {
    ...
    let (max_prob, class) = row[4..(classes + 4)].iter().enumerate().fold(
        (f32::MIN, 0),
        |(max_prob, max_class), (i, &prob)| {
            if prob > max_prob {
                (prob, i as u16)
            } else {
                (max_prob, max_class)
...
fn sorted_boxes_filtered_by_class(bboxes: &Vec<BBox>, class: u16) -> Vec<BBox> {
    let class_bboxes: Vec<BBox> = bboxes
        .iter()
